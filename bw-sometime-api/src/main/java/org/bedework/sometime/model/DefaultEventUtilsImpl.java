...
import org.bedework.sometime.IAffiliationSource;
...
public class DefaultEventUtilsImpl implements IEventUtils {
	 ...
	 */
	private static final String ICAL_DATETIME_FORMAT = "yyyyMMdd'T'HHmmss'Z'";

	/**
	 * {@link ProdId} attached to {@link Calendar}s sent to the CalDAV server by the Scheduling Assistant.
	 */
	private static final ProdId PROD_ID = new ProdId("-//jasig.org//Jasig Scheduling Assistant 1.1//EN");
	...
	private String eventClassForPersonOwners = Clazz.VALUE_CONFIDENTIAL;
	private String eventClassForResourceOwners = Clazz.VALUE_PUBLIC;
	 ...
	 * @param val
	 */
	public DefaultEventUtilsImpl(IAffiliationSource val) {
		affiliationSource = val;
	}
	/**
	...
	/**
	 * @param val the eventClassForPersonOwners to set
	 */
	public void setEventClassForPersonOwners(String val) {
		eventClassForPersonOwners = val;
	}
	/**
	 * @param val the eventClassForResourceOwners to set
	 */
	public void setEventClassForResourceOwners(String val) {
		eventClassForResourceOwners = val;
	}
	/*
	 * (non-Javadoc)
	 ...
	 */
	@Override
	public boolean attendeeMatchesPerson(final Property attendee,
                                       final ICalendarAccount calendarAccount) {
		if (null == attendee) {
			return false;
		}

		final var cn = attendee.getParameter(Cn.CN);
		if (null == cn) {
			return false;
		}

		final var cnResult = cn.getValue()
													 .equals(calendarAccount.getDisplayName());

		final var mailTo = emailToURI(calendarAccount.getEmailAddress());
		final var mailResult = attendee.getValue().equals(mailTo.toString());
	...
	public VEvent constructAvailableAppointment(
		...
		try {
			...
			final var eprops = event.getProperties();
			eprops.add(new DtStart(new DateTime(convertToICalendarFormat(block.getStartTime()))));
			eprops.add(new DtEnd(new DateTime(convertToICalendarFormat(block.getEndTime()))));

			final var ownerOrganizer =
							constructOrganizer(owner.getCalendarAccount());
			eprops.add(ownerOrganizer);

			final var visitorAttendee =
							constructSchedulingAssistantAttendee(
											visitor.getCalendarAccount(),
											AppointmentRole.VISITOR);
			eprops.add(visitorAttendee);
			final var ownerAttendee =
							constructSchedulingAssistantAttendee(
											owner.getCalendarAccount(),
											AppointmentRole.OWNER);
			eprops.add(ownerAttendee);
			...
			eprops.add(new VisitorLimit(block.getVisitorLimit()));

			final var title = new StringBuilder()
							.append(owner.getPreference(Preferences.MEETING_PREFIX));

			// update title with visitor name and add description only if visitorLimit == 1
			if (block.getVisitorLimit() == 1) {
				title.append(" with ")
						 .append(visitor.getCalendarAccount().getDisplayName());

				// build event description
				final var description = new Description(eventDescription);
				eprops.add(description);
			} 

			// finally add meeting title
			eprops.add(new Summary(title.toString()));

			if (owner.getCalendarAccount() instanceof IDelegateCalendarAccount) {
				eprops.add(new Clazz(eventClassForResourceOwners));
			} else {
				eprops.add(new Clazz(eventClassForPersonOwners));
			}
			...
			if (StringUtils.isNotBlank(blockMeetingLocationOverride)) {
				eprops.add(new Location(blockMeetingLocationOverride));
			} else {
				...
				if (StringUtils.isNotBlank(preferredLocation)) {
					eprops.add(new Location(preferredLocation));
				}
			}

			// add CONFIRMED status
			eprops.add(ImmutableStatus.VEVENT_CONFIRMED);

			// lastly we must add a UID
			eprops.add(generateNewUid());
	...
	/**
	 * Called by {@link #constructAvailableAppointment(AvailableBlock, ScheduleOwner, IScheduleVisitor, String)};
	 ...
	 * 
	 * @see IEventUtils#constructSchedulingAssistantAttendee(ICalendarAccount, AppointmentRole)
	 */
	@Override
	public Attendee constructSchedulingAssistantAttendee(
		...
		return new Attendee(parameterList,
												emailToURI(calendarAccount.getEmailAddress()));
	}
	...
	public Organizer constructOrganizer(ICalendarAccount calendarAccount) {
		...
		return new Organizer(parameterList,
												 emailToURI(calendarAccount.getEmailAddress()));
	}
	 ...
	 */
	@Override
	public Property getAttendeeForUserFromEvent(
          final VEvent event,
		...
		final var propertyList = getAttendeeListFromEvent(event);
		for(final Object o: propertyList) {
			final var attendee = (Property) o;
	...
	@Override
	public PropertyList<Attendee> getAttendeeListFromEvent(final VEvent event) {
		if (null == event) {
	...
	@Override
	public boolean willEventCauseConflict(
          final ICalendarAccount calendarAccount, final VEvent event) {
		// check to see if the owner an attendee and has ACCEPTED
		final var ownerAttendee = getAttendeeForUserFromEvent(event,
																													calendarAccount);
		...
		if (ownerAttendee != null) {
			...
			final var p = ownerAttendee.getParameter(PartStat.PARTSTAT);
	 ...
	 */
	public boolean isAttendingMatch(final VEvent event,
                                  final IScheduleVisitor visitor,
                                  final ScheduleOwner owner) {
		// only test the appointment if it's marked as an available appointment
		if (!event.getProperties()
							.contains(SchedulingAssistantAppointment.TRUE)) {
			return false;
		}

		final var visitorMatch =
						isAttendingAsVisitor(event,
																 visitor.getCalendarAccount());
		if (!visitorMatch) {
			return false;
		}

		return isAttendingAsOwner(event, owner.getCalendarAccount());
	}

	@Override
	public boolean isAttendingAsVisitor(final VEvent event,
                                      final ICalendarAccount proposedVisitor) {
		// only test the appointment if it's marked as an available appointment
		if (!event.getProperties()
							.contains(SchedulingAssistantAppointment.TRUE)) {
			return false;
		...
		final var attendees = getAttendeeListFromEvent(event);
		// walk through attendee list
		for(final var attendee: getAttendeeListFromEvent(event)) {
			// extract UW APPOINTMENT_ROLE
			final var p = attendee.getParameter(AppointmentRole.APPOINTMENT_ROLE);
			if (null != p) {
				final var role = new AppointmentRole(p.getValue());
				if (role.isVisitor() &&
								attendeeMatchesPerson(attendee, proposedVisitor)) {
					return true;
				}
			}
		}
	...
	protected net.fortuna.ical4j.model.Calendar wrapInternal(ComponentList components) {
		...
		result.getProperties().add(ImmutableVersion.VERSION_2_0);
	...
	protected VEvent convertBlockToReflectionEvent(final AvailableBlock block) {
		final Date blockStartTime = DateUtils.truncate(block.getStartTime(), Calendar.DATE);
		final DtStart start = new DtStart(new net.fortuna.ical4j.model.Date(blockStartTime));
		final DtStamp stamp = new DtStamp(new net.fortuna.ical4j.model.DateTime(new Date()));
		final Date blockEndTime = DateUtils.addDays(blockStartTime, 1);
		final DtEnd end = new DtEnd(new net.fortuna.ical4j.model.Date(blockEndTime));

		final var properties = new PropertyList<>();
		...
		properties.add(ImmutableClazz.PRIVATE);
		properties.add(new Sequence(0));

		properties.add(ImmutableTransp.TRANSPARENT);
